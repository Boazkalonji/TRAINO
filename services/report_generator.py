...
def generer_pdf_local_ticket(jasper_filename, output_filename, db_config, parameters=None):
    """
    Appelle jasperstarter en utilisant le chemin d'accès direct.
    """
    ...
    # L'option de JasperStarter pour les paramètres est -P nom=valeur
        for key, value in parameters.items():
            ...
            # Pour le chemin (P_MEDIA_ROOT), l'ajout de guillemets doubles est crucial
            # car les chemins Windows peuvent contenir des espaces ou des caractères spéciaux.
            if key == 'P_MEDIA_ROOT':
                # Notez que nous utilisons des guillemets doubles dans la chaîne
                param_args.extend(['-P', f'{key}="{value}"'])

            elif key == 'P_ID_CLIENT':
                param_args.extend(['-P', f'{key}={str(value)}(l)'])
    ...
    command = [
        ...
        '-Djava.awt.headless=true',
        '-jar', 
        JASPERSTARTER_JAR_PATH, 

        'pr', # Process Report
        jasper_path, # <-- Le chemin du JRXML

        '-f', 'pdf', # <-- Format de sortie (DOIT ÊTRE DEVANT LES ARGS DB/PARAMS)
        '-o', output_path_base, # <-- Chemin de sortie (DOIT ÊTRE DEVANT LES ARGS DB/PARAMS)

        *db_args, # <-- Arguments de connexion
        *param_args, # <-- Paramètres (P_ID_CLIENT=17)
        '-r', IMAGE_RESOURCE_DIR, # <-- Chemin des ressources
    ]
    ...
    try:
        ...
        # *** AJOUTER CES LIGNES POUR CAPTURER LES ERREURS SILENCIEUSES ***
        print(f"JasperStarter STDOUT (Sortie Std) : {result.stdout}")
        print(f"JasperStarter STDERR (Erreur Std) : {result.stderr}")
        # ***************************************************************
